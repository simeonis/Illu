Shader "Hidden/Outline"
{
    ...
    float4 FullScreenPass(Varyings varyings) : SV_Target
    {
        ...
        if (Luminance(outline.rgb) < luminanceThreshold && Luminance(outline.rgb) > _minThreshold) 
        {
            ...
            for (int i = 0; i < MAXSAMPLES; i++)
            {
                ...
                if (Luminance(neighbour) > luminanceThreshold && Luminance(neighbour) > _minThreshold)
                {
